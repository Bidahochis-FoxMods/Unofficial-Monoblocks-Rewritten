...
public class GreenNameSignItem extends Item {
   public GreenNameSignItem() {
      this.setCreativeTab(Monoblocks.monoblocksSignage);
      this.setUnlocalizedName("greennamesign");
      this.setTextureName("monoblocks:greennamesign");
   }
   ...
   public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float clickX, float clickY, float clickZ) {
      if (side == 0) {
         ...
         if (!player.canPlayerEdit(x, y, z, side, stack)) {
            return false;
         } else if (!SignReg.GreenNameSign.canPlaceBlockAt(world, x, y, z)) {
            return false;
         } else {
            world.setBlock(x, y, z, SignReg.GreenNameSign, 0, 3);
            SignReg.GreenNameSign.onBlockPlacedBy(world, x, y, z, player, stack);
