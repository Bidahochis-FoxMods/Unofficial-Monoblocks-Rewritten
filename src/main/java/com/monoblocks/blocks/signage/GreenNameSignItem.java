...
public class GreenNameSignItem extends Item {
   public GreenNameSignItem() {
      ...
      this.func_77655_b("greennamesign");
      this.func_111206_d("monoblocks:greennamesign");
   }
   ...
   public boolean func_77648_a(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float clickX, float clickY, float clickZ) {
      if (side == 0) {
         ...
         if (!player.func_82247_a(x, y, z, side, stack)) {
            return false;
         } else if (!SignReg.GreenNameSign.func_149742_c(world, x, y, z)) {
            return false;
         } else {
            world.func_147465_d(x, y, z, SignReg.GreenNameSign, 0, 3);
            SignReg.GreenNameSign.func_149689_a(world, x, y, z, player, stack);
