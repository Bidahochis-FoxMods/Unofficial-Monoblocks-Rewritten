...
public class GuiWhiteSign extends GuiScreen {
   ...
   public void func_73863_a(int mouseX, int mouseY, float something) {
      ...
      for(int i = 0; i < this.text.length; ++i) {
         this.field_146289_q.func_78276_b((lum >= 35.0F ? EnumChatFormatting.BLACK : (lum >= 31.0F ? EnumChatFormatting.GRAY : EnumChatFormatting.WHITE)) + (i == this.currentLine ? "> " : "") + this.text[i] + (i == this.currentLine ? " §r" + (lum >= 35.0F ? EnumChatFormatting.BLACK : (lum >= 31.0F ? EnumChatFormatting.GRAY : EnumChatFormatting.WHITE)) + "<" : ""), k - this.field_146289_q.func_78256_a((i == this.currentLine ? "> " : "") + this.text[i] + (i == this.currentLine ? " <" : "")) / 2 + 51, l + 4 + 10 * i, 0);
      }
