   ...
   public void func_147500_a(TileEntity te, double x, double y, double z, float scale) {
      ...
      if (strings != null && strings.length > 0) {
         ...
         for(int i = 0; i < strings.length; ++i) {
            fr.func_78276_b((lum >= 35.0F ? EnumChatFormatting.BLACK : (lum >= 31.0F ? EnumChatFormatting.GRAY : EnumChatFormatting.WHITE)) + strings[i], -fr.func_78256_a(strings[i]) / 2 + 40, 10 * i, 0);
         }
      }
